...
/**
 * <p>
 * Main class used to check if the new value of the element <b>{@code <eadid>}</b>,
 * is not already used for another <i>EAD/apeEAD</i> of the same institution.
 * </p>
 * <p>
 * Also is used to check if the new value of the element <b>{@code <recordid>}</b>,
 * is not already used for another <i>EAC-CPF/apeEAC-CPF</i> of the same
 * institution.
 * </p><br/>
 * <p>
 ...
 * <p>
 *  <ul>
 *   <li>The user is <b>editing an apeEAD</b>, of type <i>Finding aid</i>, and
 *   	has changed the content of the element <i>{@code <eadid>}</i>.</li>
 *   <li>The user has <b>uploaded a file</b> (an <i>EAD/apeEAD</i> of any type
 *   	or an <i>EAC-CPF/apeEAC-CPF</i>) and the field used to <b>identify</b>
 *   	it (<i>{@code <eadid>}</i> or <i>{@code <recordid>}</i>) <b>is empty</b>
 *   	and the system has asked the user in order to provide it.</li>
 *   <li>The user has <b>uploaded a file</b> (an <i>EAD/apeEAD</i> of any type
 *   	or an <i>EAC-CPF/apeEAC-CPF</i>) and the field used to <b>identify</b>
 *   	it (<i>{@code <eadid>}</i> or <i>{@code <recordid>}</i>) <b>is already
 *   	in use</b> for another file (of the same type) previously uploaded to
 *   	the institution.</li>
 *  </ul>
 * </p><br/>
 * <p>
...
public class GenerateEadidResponseJSONAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {
	...
	/**
	 * <p>
	 * Variable to store the identifier in database of the current EAD/apeEAD
	 * or the EAC-CPF/apeEAC-CPF which is edited, filled or changed.
	 * </p>
	 */
	 ...
	 * </p>
	 * <p>
	 * This method is called thought the Struts action <i>"generateEadidResponseJSON"</i>
	 ...
	 * type of the file (which is in <i>{@link GenerateEadidResponseJSONAction#type}</i>),
	 * is checked if the new identifier value is available or not, sending an
	...
	/**
	 * <p>
	 * This method is called thought the Struts action <i>"generateEadidResponseJSONWhenEdit"</i>
	 ...
	 * its identifier is available in <i>{@link GenerateEadidResponseJSONAction#fileId}</i>).
	 * </p>
	 * <p>
	 * Once the current institution is obtained, and in combination with the
	 * type of the file (as mentioned <i>Finding aid</i>), is checked if the
	...
	/**
	 * <p>
     * This method is called from <i>"{@link GenerateEadidResponseJSONAction#execute()}"</i>
     * and from <i>"{@link GenerateEadidResponseJSONAction#executeWithoutFile()}"</i>.
	 * </p>
     * <p>
     * It is used to check if the new identifier, which is in <i>{@link GenerateEadidResponseJSONAction#fileId}</i>,
     * it is available to be used, is empty/blank or is already in use.
     * </p>
     * <p>
     ...
     * between the client and the server if they are not escaped. So previous
     * to make any of the checks the identifiers are unescaped.
     * </p>
	 *
     ...
     * <p>
     *  <ul>
     *   <li><b>There is another file with this ID</b> - If already exists
     *   	another file for that institution, with same type, which have
     *   	the same value as identifier.</li>
     *   <li><b>ID cannot be empty</b> - If the new identifier filled is empty
     *   	or blank.</li>
