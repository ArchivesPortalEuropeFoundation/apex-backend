...
//	public static final DecimalFormat SIMPLE_NUMBERFORMAT = new DecimalFormat("000"); 
    ...
    static {
    	try {
    		...
    		languageExpression = XPATH.compile("/ead:ead/ead:eadheader/ead:profiledesc/ead:langusage/ead:language/@langcode");
    		archdescLangmaterialExpression = XPATH.compile("/ead:ead/ead:archdesc/ead:did/ead:langmaterial/ead:language/@langcode");
    		archdescUnitidExpression = XPATH.compile("/ead:ead/ead:archdesc/ead:did/ead:unitid/text()");
    		fondTitleExpression = XPATH.compile("/ead:ead/ead:eadheader/ead:filedesc/ead:titlestmt/ead:titleproper[1]//text()");
    		...
    		//daoRoleAttributeExpression = XPATH.compile("./ead:dao[not(@xlink:title='thumbnail')]/@xlink:role");
    		...
    		cOtherUnitidExpression = XPATH.compile("./ead:unitid[@type='former call number' or @type='file reference']/text()");
    		...
    		cDidOtherExpression = XPATH.compile("/ead:c/ead:did/node()[not(name()='unittitle' or name()='unitid' or name()='unitdate' or name()='dao')]//text()");
    		...
    		displayIntroExpression = XPATH.compile("/ead:ead/ead:archdesc/ead:scopecontent or" +
    				"/ead:ead/ead:archdesc/ead:bioghist or " +
    				...
    				"/ead:ead/ead:archdesc/ead:arrangement or " +
    				"/ead:ead/ead:archdesc/ead:fileplan or " +
    				"/ead:ead/ead:archdesc/ead:accessrestrict or " +
    				"/ead:ead/ead:archdesc/ead:userestrict or " +
    				"/ead:ead/ead:archdesc/ead:phystech or " +
     				"/ead:ead/ead:archdesc/ead:otherfindaid or " +
    				"/ead:ead/ead:archdesc/ead:relatedmaterial or " +
    				"/ead:ead/ead:archdesc/ead:separatedmaterial or " +  
    				"/ead:ead/ead:archdesc/ead:altformavail or " +
     				"/ead:ead/ead:archdesc/ead:originalsloc or " +
    				"/ead:ead/ead:archdesc/ead:bibliography or " +
    				"/ead:ead/ead:archdesc/ead:prefercite or " +      	
     				"/ead:ead/ead:archdesc/ead:odd or " +
    				"/ead:ead/ead:archdesc/ead:controlaccess or " +
    				"/ead:ead/ead:archdesc/ead:did/ead:note");
    		displayDidExpression = XPATH.compile("/ead:ead/ead:archdesc/ead:did/ead:unitid or" +
    				...
    				"/ead:ead/ead:archdesc/ead:did/ead:physdesc or " +
    				"/ead:ead/ead:archdesc/ead:did/ead:dao");
	...
	public long parseHeader(EadContent eadContent) throws Exception {
		...
		try {
            ...
            LOG.info(ead.getEadid() + " start index");
			isFinalPath = true;
	...
	private long extractGeneralData(Document doc, EadContent eadContent) throws XPathExpressionException {
		...
		if (StringUtils.isNotBlank(countryString)){
			try {
				...
				throw new APEnetRuntimeException("Extract General Data: Don't exist the countrycode into the file. Warning: "+ ex.getMessage());
			}
		}else {
			throw new APEnetRuntimeException("COUNTRY: There is not countrycode");
		}
		// Get out the language. @LANGCODE of all <language> node of <langusage>.
	...
	public long parseCLevel(IndexData indexData) throws Exception   {
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		factory.setNamespaceAware(true);
		DocumentBuilder builder = factory.newDocumentBuilder();
	...
	private long parseCLevel(Document document, IndexData indexData) throws Exception {
		...
		//String roleDao = (String) daoRoleAttributeExpression.evaluate(didNode, XPathConstants.STRING);
		Set<String> roleDao = getTextsWithoutMultiplity((NodeList) daoRoleAttributeExpression.evaluate(didNode, XPathConstants.NODESET));
		String clevelLangmaterial = getText((NodeList) cLangmaterialExpression.evaluate(didNode, XPathConstants.NODESET));
		String otherinfo = "";
		String unittitle = "";
		...
		if (hasUnittitle){
			...
			otherinfo += WHITE_SPACE + getText((NodeList) cOtherUnittitleExpression.evaluate(didNode, XPathConstants.NODESET));
		}
		...
		if (StringUtils.isNotBlank(unitdate)){
			otherinfo += WHITE_SPACE + getText((NodeList) cOtherUnitdateExpression.evaluate(didNode, XPathConstants.NODESET));
		}
			...
			if (StringUtils.isNotBlank(unitdatenormal)) {
				try {
					if (unitdatenormal.contains("/")) {
						...
						String ed = "";
						String od = "";
						if (list.length == 2){// The meaning is: startdate/enddate
							sd = list[0].trim();
		...
		//but if this c-level doesn't contain langmaterial Node, then langmaterial content of archdesc Node will be indexed.
		String langmaterial="";
		...
		indexNode(indexData, unitid, otherunitid, scopecontent, unittitle, attributeLevel, startdate, enddate, alterdate, hasDao, roleDao, otherinfo, langmaterial);
		return numberOfDaos.longValue();
	}
	...
	private void indexNode(IndexData indexData, String unitid, String otherunitid, String scopecontent, String title, String level, String sdate, String edate, String alterdate, boolean dao, Set<String> roleDao, String otherinfo, String langmaterial) throws MalformedURLException, SolrServerException, IOException {
	...
	public void commitAll(EADCounts eadCounts) throws MalformedURLException, SolrServerException, IOException {
		...
		if (!isFinalPath) {
			...
			String finalpath2 = APEnetUtilities.FILESEPARATOR + theCountry + APEnetUtilities.FILESEPARATOR + institutionStr + APEnetUtilities.FILESEPARATOR + "FA" + APEnetUtilities.FILESEPARATOR + foldername + APEnetUtilities.FILESEPARATOR + foldername + ".xml";
			if (xmlType == XmlType.EAD_HG || xmlType == XmlType.EAD_SG) {
				finalpath2 = APEnetUtilities.FILESEPARATOR + theCountry + APEnetUtilities.FILESEPARATOR + institutionStr + APEnetUtilities.FILESEPARATOR + "HG" + APEnetUtilities.FILESEPARATOR + foldername + ".xml";
			}
	 ...
	 * temporal directory and finally deletes the source file if everything is ok
	...
	private void insertFileToRepository(String srcFilePath, String destFilePath) throws IOException {
		...
		String trash = APEnetUtilities.getDashboardConfig().getTempAndUpDirPath() + APEnetUtilities.FILESEPARATOR  + "Trash" + APEnetUtilities.FILESEPARATOR;
