...
/**
 * Edit Archival Landscape Action User: Jara Alvarez Date: Sep 8th, 2010
 * 
 */
public class EditAction extends eu.apenet.dashboard.archivallandscape.AbstractEditAction {
	...
	public void setArchivalInstitutionsToDelete(List<ArchivalInstitution> archivalInstitutionsToDelete) {
	...
	/**
	 * When the AL is being adding the first time. It controls the first form
	 * when it's pushed the add button.
	 * 
	 * @return String
	 */
	...
	/**
	 * Action which controls the moving up of an Institution while it's edited
	 * 
	 * @return String
	 */
	...
	/**
	 * Action which controls the moving down of an Institution while it's
	 * edited.
	 * 
	 * @return String
	 */
	...
	/**
	 * Method which control the second form when it's pressed the Finish button.
	 * 
	 * @return String
	 * @throws IOException
	 ...
	 */
	public String appendArchivalLandscape() throws IOException, XMLStreamException, ParserConfigurationException,
...
//			}
			...
			try {
				...
				for (int i = 0; i < cList.getLength() && nodeTemp == null; i++) {
					if (cList.item(i).hasAttributes() && cList.item(i).getAttributes().getNamedItem("id") != null
							&& cList.item(i).getAttributes().getNamedItem("id").getNodeValue().equals(getALElement())) {
						...
						if (childrenBefore.item(i).getNodeName().equals("did")) {
							...
							for (int x = 0; x < children.getLength(); x++) {
								if (children.item(x).getNodeName().equals("unittitle")) {
									this.elementLanguages.add(children.item(x).getAttributes().getNamedItem("type")
											.getTextContent()
											+ " - " + children.item(x).getTextContent());
								}
							}
						}
					}
	...
	public String finishAddArchivalLandscape() {
		...
		try {
			...
			if (resultStoreArchives.equals("success")) {
				...
				log.info("EditAction: archival landscape of " + a.getmyCountry() + " renamed to" + a.getmyCountry()
						+ "_old.xml");
			...
			ddbbResult = false;
			log.error("The new archival landscape of " + a.getmyCountry()
					+ " could not be overwrited requested by edition process", e);
			...
			if (!ddbbResult) {
				log.error("Some operation was not correct in overwriting the AL of the " + a.getmyCountry()
						+ " requested by edition process. Rollbacking the whole transaction process");
				try {
					...
					// Rollback the repository files AL for this country. Delete
					// the one generated.
					...
					for (int i = 0; i < this.archivalInstitutionsToDelete.size(); i++) {
						...
						String tmpDirOldPath = APEnetUtilities.getDashboardConfig().getTempAndUpDirPath()
								+ APEnetUtilities.FILESEPARATOR + this.archivalInstitutionsToDelete.get(i).getAiId()
								+ "_old";
						String tmpDirPath = APEnetUtilities.getDashboardConfig().getTempAndUpDirPath()
								+ APEnetUtilities.FILESEPARATOR + this.archivalInstitutionsToDelete.get(i).getAiId();
						File tmpDirFile = new File(tmpDirOldPath);
						if (tmpDirFile.exists()) {
							log.debug("Renaming the tmp folder from the repository related the institution: "
									+ this.archivalInstitutionsToDelete.get(i));
							tmpDirFile.renameTo(new File(tmpDirPath));
						}
						// Rename the files in Country Repo folder
						String repoDirOldPath = APEnetUtilities.getConfig().getRepoDirPath()
								+ APEnetUtilities.FILESEPARATOR + a.getmyCountry() + APEnetUtilities.FILESEPARATOR
								+ this.archivalInstitutionsToDelete.get(i).getAiId() + "_old";
						String repoDirPath = APEnetUtilities.getConfig().getRepoDirPath()
								+ APEnetUtilities.FILESEPARATOR + a.getmyCountry() + APEnetUtilities.FILESEPARATOR
								+ this.archivalInstitutionsToDelete.get(i).getAiId();
						File repoDirFile = new File(repoDirOldPath);
						if (repoDirFile.exists()) {
							log.debug("Renaming the repo folder from the repository related to the institution: "
									+ this.archivalInstitutionsToDelete.get(i));
					...
					log.error("FATAL ERROR. The rollback of index or in repository could not be done successfully. A manually review of the AL of: "
							+ a.getmyCountry() + " must be done. Error: " + e);
				}
				...
				try {
					this.setArchivalInstitutionsToDelete(a.getArchivalInstitutionsToDelete());
					// Delete the temporary file (_old) country AL created in
					// overwriting
					...
					for (int i = 0; i < this.archivalInstitutionsToDelete.size(); i++) {
						...
						String tmpDirOldPath = APEnetUtilities.getDashboardConfig().getTempAndUpDirPath()
								+ APEnetUtilities.FILESEPARATOR + "tmp" + APEnetUtilities.FILESEPARATOR
								+ this.archivalInstitutionsToDelete.get(i).getAiId() + "_old";
						File tmpDirFile = new File(tmpDirOldPath);
						if (tmpDirFile.exists()) {
							log.debug("Renaming the tmp folder from the repository related the institution: "
					...
					log.error("The temporary file" + pathCountryALOld + " or the ones related to the institutions: "
							+ this.archivalInstitutionsToDelete
							+ " could not have been deleted. It should remove it manually", e);
				}
			}
		}
		return result;
	 ...
	 * @throws TransformerException
	 */
	public String modifyArchivalLandscape() throws ParserConfigurationException, SAXException, IOException,
		...
		String path = a.getmyPath(a.getmyCountry()) + "tmp" + APEnetUtilities.FILESEPARATOR + a.getmyCountry()
				+ "AL.xml";
		fillCountryCIdentifier(path);
		if (getALElement() != null) {// If there's some element selected in the
										// list
			if (remove(getALElement(), list)) {
				list = EditArchivalLandscapeLogic.navigate(false);
				parseList(list, null);
				return SUCCESS;
			} else {// Some errors occurs in the deletion process (remove
					// returns FALSE)
				list = EditArchivalLandscapeLogic.navigate(false);
				parseList(list, null);
				addActionMessage(getText("al.message.errorDeletion"));
				log.error("The deletion of the item: " + getALElement() + " from the AL of " + getCountry()
						+ " could not be done");
				return INPUT;
			}
				...
				for (int i = 0; i < listTemp.getLength(); i++) {
					...
					if (listTemp.item(i).getNodeName().equals("did")) {
						...
						for (int x = 0; x < unittitles.getLength(); x++) {
							if (unittitles.item(x).hasAttributes()
									&& unittitles.item(x).getAttributes().getNamedItem("type") != null
									&& unittitles.item(x).getAttributes().getNamedItem("type").getTextContent()
											.toLowerCase().equals(this.languageTarget.toLowerCase())) {
						...
						if (nodeToBeReplaced != null) {// if edition of language
														// is not present...
