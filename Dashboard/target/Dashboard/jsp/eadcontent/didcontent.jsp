...
<c:when test="${print=='true'}">
	...
	<apenet:context eadContent="${eadContent}" country="${localizedCountryName}" />
</div>
</c:when>
<c:otherwise>
<c:set var="printUrl" value="${pageContext.request.contextPath}/displayEadDidContent.action?fileId=${fileId}&xmlTypeId=${xmlTypeId}&print=true"/>
<script>
    $(document).ready(function() {
...
<dashboard:ead type="didcontent" xml="${eadContent.xml}" />
</div>
