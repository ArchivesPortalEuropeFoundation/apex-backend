...
/**
 * This class and its methods has been imported to the Portal from Dashboard done by Eloy
 * 
	User: Eloy García
 * Date: Sep 23d, 2010
 */
					...
					case XMLEvent.START_ELEMENT:
						for(int i=0;i<pathElements.length;i++){
							if(input.getLocalName().equals(pathElements[i])){ 
								...
								if(input.getLocalName().trim().equals(pathElements[pathElements.length-2])){
									found = true;
								}
							}
						}
						...
						if (oneOfTwoFound == 1){
							...
							if (isInsideElement) {
								if (shouldHaveText) {
									...
									// The repositorguide should't have text
									if (input.hasText()) {
