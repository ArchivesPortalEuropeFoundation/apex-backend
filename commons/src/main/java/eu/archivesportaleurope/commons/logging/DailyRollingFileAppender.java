 ...
 *
 * <p>
 * The rolling schedule is specified by the <b>DatePattern</b>
 * option. This pattern should follow the {@link SimpleDateFormat} conventions.
 * In particular, you <em>must</em> escape literal text within a pair of single
 ...
 * <b>DatePattern</b> option. The text before the colon is interpeted as the
...
public class DailyRollingFileAppender extends FileAppender {
     ...
     * GZIP
         */
        File logDir = target.getParentFile();
        GZCompressAction gzCompressAction = new GZCompressAction(target, new File(logDir, target.getName() + ".gz"), true);
        gzCompressAction.run();
        File[] fileArray = logDir.listFiles(new FileDateFilenameFilter(fileName));
        List<File> files = Arrays.asList(fileArray);
        Collections.sort(files, new FileDateComparator(sdf, fileName));
        for (int i = maxBackupIndex; i < files.size(); i++) {
            files.get(i).delete();
        }
    }
...
class FileDateFilenameFilter implements FilenameFilter {

    private String filePrefix;

    public FileDateFilenameFilter(String filename) {
        File file = new File(filename);
...
class FileDateComparator implements Comparator<File> {
    ...
    public FileDateComparator(SimpleDateFormat simpleDateFormat, String filename) {
    ...
    public int compare(File file1, File file2) {
        String fileName1 = getName(file1);
        String fileName2 = getName(file2);
        try {
            Date file1Date = simpleDateFormat.parse(fileName1);
            Date file2Date = simpleDateFormat.parse(fileName2);
